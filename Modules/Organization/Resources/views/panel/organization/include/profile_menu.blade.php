...
<li class="cart-list onhover-dropdown">
    ...
    <div class="onhover-dropdown-show dropdown-list-style white-bg">
        <!-- User info -->
        <a href="{{ route('organization.profile') }}" class="user-sub-info border-0 pb-0">
            <div class="user-img">
                <img src="{{ showImage(auth()->user()->image->original ?? '', 'default-1.jpeg') }}"
                    class="img-cover" alt="{{ auth()->user()->name ?? '' }}">
            </div>
        ...
        <ul class="profileListing">
            <li class="list">
                <a class="list-items" href="{{ route('organization.dashboard') }}">
                    <i class="ri-dashboard-line"></i>{{ ___('organization.Dashboard') }}
                </a>
            </li>
            <li class="list">
                <a class="list-items" href="{{ route('organization.profile') }}">
                    <i class="ri-contacts-line"></i>{{ ___('frontend.My Profile') }}
                </a>
            </li>
            <li class="list">
                <a class="list-items" href="{{ route('organization.courses') }}">
                    <i class="ri-book-open-line"></i>
                    {{ ___('sidebar.My Courses') }}
                </a>
            </li>
            <li class="list">
                <a class="list-items" href="{{ route('organization.setting', ['edit']) }}">
        ...
        <form action="{{ route('organization.logout') }}" method="POST" id="logoutForm">
            @csrf
        </form>
    </div>
</li>
