...
class PaymentMethodController extends Controller
{
    ...
    {
        try {
            $data['title'] = ___('organization.Payment Account List'); // title
            $data['payment_methods'] = $this->instructorPaymentMethodRepository->filter(['user_id' => auth()->user()->id])->get(); // payment methods
            return view('organization::panel.organization.financial.payout_settings', compact('data'));
        } catch (\Throwable $th) {
    ...
    {
        try {
            $data['url'] = route('organization.payment_method.store'); // url
            $data['title'] = ___('organization.Add Payment Account'); // title
            $data['payment_methods'] = $this->paymentMethodRepository->model()->where('status_id', 1)->get(); // payment methods
            @$data['button'] = ___('common.Save');
            $html = view('organization::panel.organization.modal.payment.create_payment_account', compact('data'))->render(); // render view
    ...
    public function edit($id)
    {
        ...
        try {
            ...
            $data['url'] = route('organization.payment_method.update', $id); // url
            $data['title'] = ___('organization.Edit Payment Account'); // title
            $data['payment_methods'] = $this->paymentMethodRepository->model()->where('status_id', 1)->get(); // payment methods
            @$data['button'] = ___('common.Save');
            $html = view('organization::panel.organization.modal.payment.edit_payment_account', compact('data'))->render(); // render view
            return $this->responseWithSuccess(___('alert.data_retrieve_success'), $html); // return success response
        } catch (\Throwable $th) {
            return $this->responseWithError(___('alert.something_went_wrong_please_try_again'), [], 400); // return error response
        }
