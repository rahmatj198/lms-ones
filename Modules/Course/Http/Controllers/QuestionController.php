...
class QuestionController extends Controller
{
    ...
    public function questionList($quiz_id)
    {
        try {
            $data['quizzes'] = $this->lesson->model()->where('id', $quiz_id)->first(); // data
            if (!$data['quizzes']) {
                return redirect()->back()->with('danger', ___('alert.Quiz not found'));
            }
            $data['questions'] = $data['quizzes']->questions()->latest()->paginate(10);
            $data['title'] = ___('course.Question Lists'); // title
            return view('course::questions.index', compact('data')); // view
        } catch (\Throwable $th) {
            return redirect()->back()->with('danger', ___('alert.something_went_wrong_please_try_again'));
        }
    ...
    public function create($lesson_id)
    {
        ...
        try {
            $data['course'] = $this->lesson->model()->where('id', $lesson_id)->first(); // data
            if (!$data['course']) {
                return $this->responseWithError(___('alert.Quiz_not_found'), [], 400); // return error response
            }
            $data['url'] = route('admin.question.store', $lesson_id); // url
            $data['title'] = ___('course.Create Course Question'); // title
            @$data['button'] = ___('common.Save');
            $html =  view('course::questions.modal.create', compact('data'))->render();
            return $this->responseWithSuccess(___('alert.data_retrieve_success'), $html); // return success response
        } catch (\Throwable $th) {
            return $this->responseWithError(___('alert.something_went_wrong_please_try_again'), [], 400); // return error response
        }
    ...
    {
        try {
            $data['lesson'] = $this->lesson->model()->where('id', $lesson_id)->first(); // data
            if (!$data['lesson']) {
    ...
    {
        try {
            $data['question'] = $this->question->model()->where('id', $id)->first(); // data
            if (!$data['question']) {
                return $this->responseWithError(___('alert.Question_not_found'), [], 400); // return error response
            }
            $data['url'] = route('admin.question.update', $id); // url
            $data['title'] = ___('course.Edit Course Question'); // title
            @$data['button'] = ___('common.Save');
            $html =  view('course::questions.modal.edit', compact('data'))->render();
            return $this->responseWithSuccess(___('alert.data_retrieve_success'), $html); // return success response
        } catch (\Throwable $th) {
            return $this->responseWithError(___('alert.something_went_wrong_please_try_again'), [], 400); // return error response
        }
